...
export function CategoryManager({
  ...
  // 渲染子分類（中類）
  const renderChild = (node: CategoryTreeNode) => {
    const isEditing = editingId === node.id
    const isMenuOpen = menuOpenId === node.id

    return (
      <div key={node.id} className="flex items-center gap-2 py-2 px-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg">
        {isEditing ? (
          <>
            <input
              type="text"
          ...
          <>
            ...
            <div className="relative">
              <button
                ...
                className="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 rounded transition-colors focus:outline-none"
              >
              ...
              {isMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 w-32 bg-white dark:bg-gray-700 rounded-lg shadow-lg border dark:border-gray-600 z-20 overflow-hidden">
                    <button
  ...
  // 渲染大類（手風琴樣式）
  const renderMainCategory = (node: CategoryTreeNode) => {
    ...
    return (
      <div key={node.id} className="border dark:border-gray-700 rounded-lg overflow-hidden mb-2">
        {/* 大類標題 */}
        <div className={`flex items-center gap-2 px-4 py-3 bg-gray-50 dark:bg-gray-700/50 ${isMenuOpen ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
          {isEditing ? (
            <>
              <input
                type="text"
            ...
            <>
              ...
              <div className="relative">
                ...
                {isMenuOpen && (
                  <>
                    <div className="fixed inset-0 z-10" onClick={() => setMenuOpenId(null)} />
                    <div className="absolute right-0 top-full mt-1 w-36 bg-white dark:bg-gray-700 rounded-lg shadow-lg border dark:border-gray-600 z-20 overflow-hidden">
                      <button
                        onClick={() => handleStartEdit(node)}
                        className="w-full px-4 py-2.5 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 flex items-center gap-2"
                      >
                        <svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                      ...
                      <button
                        onClick={() => handleStartAdd(node.id, 2)}
                        className="w-full px-4 py-2.5 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 flex items-center gap-2"
                      >
  ...
  return (
    <div>
      ...
      {!loading && tree.length > 0 && (
        <div>
          {tree.map(node => renderMainCategory(node))}
        </div>
      )}
