...
src/
├── components/     # UI 組件
│   ├── CalendarView.tsx      # 日曆視圖（含日期滑動切換）
│   ├── DayDetail.tsx         # 日期明細彈窗（含語音記帳）
│   ├── CategoryPieChart.tsx  # 分類圓餅圖（月份選擇、標籤、hover、鑽取）
│   ├── MonthlyBarChart.tsx   # 月度長條圖（K/M 格式化）
...
- **日曆記帳**：點擊日期新增/編輯交易，支援語音輸入
- **快速記帳**：浮動按鈕 + 語音輸入 + 拍照
- **語音辨識**：自動解析金額、分類、備註（支援中類優先匹配）
- **預算追蹤**：設定月預算，顯示剩餘/超支
- **分類管理**：2 層分類架構（大類 > 中類），支援排序
- **統計圖表**：
  - 圓餅圖：月份切換、標籤顯示、hover 互動、點擊鑽取細項
  - 長條圖：最近 6 個月收支趨勢，Y 軸 K/M 縮寫
- **訂閱管理**：
  - 自動記帳模式：扣款日自動產生支出記錄
  - 手動確認模式：到期時彈窗提醒確認
  - 補記功能：漏記時可手動補記
...
- 訂閱自動記帳需要 App 開啟才會觸發（非伺服器推送）
- 訂閱記帳失敗時會保留重試機會（不更新 lastProcessedMonth）
